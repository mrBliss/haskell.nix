    ...
    compiler =
        let bootPkgs = with final.buildPackages; {
            ...
            ghc-patches = version: let
                ...
                ++ final.lib.optional (versionAtLeast "9.2"  && versionLessThan "9.4" && final.targetPlatform.isAarch64) ./patches/ghc/ghc-9.2-revert-linker-changes.patch
                ++ final.lib.optional (versionAtLeast "9.2"  && versionLessThan "9.4" && final.targetPlatform.isAarch64) ./patches/ghc/ghc-9.2-mmap-next.patch
                ++ final.lib.optional (versionAtLeast "9.2"  && versionLessThan "9.4" && final.targetPlatform.isAarch64) ./patches/ghc/ghc-9.2-m32_alloc.patch
